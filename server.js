...
const initializeFiles = () => {
  if (!fs.existsSync('files.json')) {
    fs.writeFileSync('files.json', '[]', 'utf8');
  }
};
initializeFiles();

const getFileColumns = (filePath) => {
  try {
...
app.delete('/files/:id', (req, res) => {
  try {
    ...
    fs.writeFileSync('files.json', JSON.stringify(updatedFiles));
...
try {
  ...
  fs.writeFileSync('charts.json', '[]');
}

app.get('/charts', (req, res) => {
  try {
    const chartsData = fs.readFileSync('charts.json', 'utf8');
...
app.post('/charts', (req, res) => {
  try {
    ...
    fs.writeFileSync('charts.json', JSON.stringify(charts));
